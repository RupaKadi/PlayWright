//Instead of taking token and other properties one by one we can store them in json file and use it in our next webpages. We will use storage state for this purpose
//We should store it at context level but not at page level
...
test.beforeAll(async({browser})=>
{
   ...
   await context.storageState({path: 'state.json'}); //this will create state.json with all details of session storage
   webContext = await browser.newContext({storageState: 'state.json'});  //creating new browser context with name webContext -to store that state.json file

})


test.only('Test end to end flow', async ()=>
    {
   
   const page = await webContext.newPage();   //same as creating new page using context. we are creating new page of webContext and this webCOntext have idea of state.json file
   await page.goto("https://rahulshettyacademy.com/client");
   const productName = page.locator('.card-body b');
